    ...
    <tr className={`border-t transition duration-150 hover:bg-blue-50 relative group`}>
      ...
      <td className="p-4 align-middle text-center relative">
        <div className="relative inline-block text-left" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div
              className="absolute right-0 top-full mt-1.5 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl ring-1 ring-black/5 z-50 animate-in fade-in zoom-in-95 duration-100"
            >
              <div className="py-1">
    ...
    <tr className="border-t transition duration-150 bg-gray-100 opacity-60 hover:opacity-100 hover:bg-gray-200">
      ...
      <td className="p-4 align-middle text-center relative">
        <div className="relative inline-block text-left" ref={menuRef}>
          ...
          {isMenuOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-52 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl ring-1 ring-black/5 z-50 animate-in fade-in zoom-in-95 duration-100">
              <div className="py-1">
...
function EmployeeOverviewModal({ open, employee, onClose }) {
  ...
  const handleAttStartDateChange = (newStartDate) => {
    setAttStartDate(newStartDate);
    if (attEndDate && newStartDate > attEndDate) {
      setAttEndDate(newStartDate);
      Swal.fire({
        icon: "warning",
        title: "Invalid Date Range",
        text: "'To' date cannot be earlier than 'From' date. 'To' date updated automatically.",
        timer: 3000,
  ...
  const handleAttEndDateChange = (newEndDate) => {
    if (attStartDate && newEndDate < attStartDate) {
      Swal.fire({
        icon: "error",
        title: "Invalid Date Range",
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="flex items-center bg-slate-100 rounded-lg px-3 py-1 border">
                  ...
                  <input type="date" value={attStartDate} onChange={(e) => handleAttStartDateChange(e.target.value)} className="bg-transparent text-sm font-semibold outline-none text-slate-700" />
                </div>
                <div className="flex items-center bg-slate-100 rounded-lg px-3 py-1 border">
                  <span className="text-xs text-slate-500 mr-2 uppercase font-bold">To</span>
                  <input type="date" value={attEndDate} min={attStartDate} onChange={(e) => handleAttEndDateChange(e.target.value)} className="bg-transparent text-sm font-semibold outline-none text-slate-700" />
                </div>
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center py-6 md:py-12">
      <div className="w-full max-w-[98%] xl:max-w-7xl mx-auto px-2 sm:px-4">
        ...
        {/* Filters Section */}
        <div className="flex flex-col xl:flex-row items-center gap-2.5 mb-8 px-4 sm:px-8">
          <div className="relative flex-[1.5] min-w-0 w-full xl:w-auto">
            ...
            <input
              ...
              className="w-full border bg-white border-gray-200 pl-9 pr-3 py-2 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 outline-none text-xs font-medium"
            />
          </div>
          <select
            value={selectedCompany}
            onChange={(e) => setSelectedCompany(e.target.value)}
            className="flex-1 min-w-0 w-full xl:w-auto bg-white border border-gray-200 px-2.5 py-2 rounded-xl shadow-sm focus:ring-2 focus:ring-blue-500 outline-none font-medium text-gray-700 text-xs truncate"
          >
          ...
          <button
            ...
            className="shrink-0 w-full xl:w-auto justify-center bg-white text-red-500 border border-red-100 px-3.5 py-2 rounded-xl font-bold flex items-center gap-1.5 hover:bg-red-50 hover:border-red-200 transition text-xs active:scale-95"
          >
            <FaUndo /> Clear
          </button>
        </div>
