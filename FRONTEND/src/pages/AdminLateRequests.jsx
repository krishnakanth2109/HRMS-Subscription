...
const AdminLateRequests = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(`/api/attendance/admin/pending-late-requests?audience=${candidateType}`);
  ...
  const fetchFullDayRequests = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(`/api/attendance/admin/full-day-requests?audience=${candidateType}`);
  ...
  const fetchCorrectionRequests = useCallback(async () => {
    ...
    try {
      const { data } = await api.get(`/api/attendance/admin/pending-corrections?audience=${candidateType}`);
  ...
  const fetchEmployeeLimits = useCallback(async (showLoader = true) => {
    if (showLoader) setLoadingLimits(true);
    try {
      const { data } = await api.get(`/api/attendance/admin/all-request-limits?audience=${candidateType}`);
