  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-800 text-white px-6 py-2 rounded-lg shadow-lg">
          Apply For Overtime
        </button>
      </div>
        ...
        <table className="min-w-full text-sm">
          ...
          <tbody>
            {overtimeList.length > 0 ? (
              ...
              <tr><td colSpan="3" className="text-center p-4 text-gray-500">No Overtime Requests Found</td></tr>
            )}
          </tbody>
        </table>
      </div>
          ...
          <motion.div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <motion.div className="bg-white w-96 p-6 rounded-xl shadow-xl" initial={{ scale: 0.7 }} animate={{ scale: 1 }} exit={{ scale: 0.7 }}>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                ...
                <button type="submit" className="bg-indigo-600 hover:bg-indigo-800 text-white px-4 py-2 rounded shadow">Submit</button>
              </form>
              <button onClick={() => setModalOpen(false)} className="mt-4 text-sm text-gray-500 underline w-full text-center">Close</button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
