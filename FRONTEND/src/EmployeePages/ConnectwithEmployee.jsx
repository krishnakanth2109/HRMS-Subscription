...
const ConnectWithEmployee = () => {
  ...
  useEffect(() => {
    ...
    socket.on('connect', () => socket.emit('authenticate', currentUserId));

    socket.on('receive_message', (messageData) => {
      if (selectedChatUser && (messageData.sender._id === selectedChatUser._id || messageData.sender === selectedChatUser._id)) {
        setDirectMessages(prev => [...prev, messageData]);
        markMessagesAsRead(messageData.sender._id || messageData.sender);
      } else {
        setChatList(prev => prev.map(u => 
      ...
      setChatList(prev => {
        const senderId = messageData.sender._id || messageData.sender;
        const filtered = prev.filter(u => u._id !== senderId);
        const sender = prev.find(u => u._id === senderId) || { _id: senderId, name: messageData.sender.name };
        return[{...sender, lastMessage: messageData.message, lastMessageTime: messageData.createdAt}, ...filtered];
      });
      playNotificationSound();
    });
    ...
    socket.on('messages_read', () => {
      setDirectMessages(prev => prev.map(msg => 
        (msg.sender._id === currentUserId || msg.sender === currentUserId) ? { ...msg, isRead: true } : msg
      ));
    });

    socket.on('message_edited', ({ messageId, newMessage }) => {
      setDirectMessages(prev => prev.map(msg => msg._id === messageId ? { ...msg, message: newMessage } : msg));
    });
    ...
    socket.on('user_typing', ({ userId }) => {
      if (selectedChatUser && userId === selectedChatUser._id) setTypingUsers(prev => new Set([...prev, userId]));
    });
  ...
  useEffect(() => {
    if (!isChatLoading && !isSwitchingUser && directChatEndRef.current) {
      setTimeout(() => directChatEndRef.current?.scrollIntoView({ behavior: "smooth" }), 100);
    }
  }, [directMessages, isChatLoading, isSwitchingUser]);
  ...
  const handleTyping = useCallback((text) => {
    ...
    if (text.trim()) {
      socketRef.current.emit('typing_start', { receiverId: selectedChatUser._id, senderId: currentUserId });
      if (typingTimeoutRef.current) clearTimeout(typingTimeoutRef.current);
      typingTimeoutRef.current = setTimeout(() => {
  ...
  const fetchDirectMessages = useCallback(async (userToFetch, isSilent = true) => {
    ...
    try {
      const { data } = await api.get(`/api/chat/history/${userToFetch._id}`);
      if (selectedChatUser?._id === userToFetch._id) setDirectMessages(data ||[]);
    } catch (err) {
      toast.error("Failed to load messages");
    } finally {
      setIsChatLoading(false);
  ...
  const markMessagesAsRead = useCallback(async (senderId) => {
    ...
    try {
      await api.put(`/api/chat/read/${senderId}`);
      setChatList(prev => prev.map(u => u._id === senderId ? { ...u, unreadCount: 0 } : u));
      setTotalUnreadCount(prev => {
        const user = chatList.find(u => u._id === senderId);
        return user ? Math.max(0, prev - (user.unreadCount || 0)) : prev;
      });
  ...
  const handleUserSwitch = useCallback(async (emp) => {
    ...
    setChatList(prev => prev.find(u => u._id === emp._id) ? prev : [emp, ...prev]);
    setIsSwitchingUser(false);
  },[selectedChatUser, isSwitchingUser, markMessagesAsRead, fetchDirectMessages]);
  ...
  const displayList = useMemo(() => {
    const term = directSearchTerm.toLowerCase();
    let list = directSearchTerm 
      ? employees.filter(e => e._id !== currentUserId && (e.name?.toLowerCase().includes(term) || e.employeeId?.toLowerCase().includes(term)))
      : chatList;
    return [...list].sort((a, b) => new Date(b.lastMessageTime || 0).getTime() - new Date(a.lastMessageTime || 0).getTime());
  }, [directSearchTerm, chatList, employees, currentUserId]);

  // INITIAL DATA LOAD - WITH GRACEFUL ERROR HANDLING
  useEffect(() => {
    const loadData = async () => {
      try {
        const empData = await getEmployees();
        setEmployees(empData.filter(e => e.isActive !== false && e._id !== currentUserId));
        
        const { data: chatData } = await api.get("/api/chat/users");
        setChatList(chatData ||[]);
        setTotalUnreadCount(chatData.reduce((acc, curr) => acc + (curr.unreadCount || 0), 0));
        
        // Isolate online users fetch so a 404 doesn't break the whole app
        try {
          const { data: onlineData } = await api.get("/api/chat/online-users");
          setOnlineUsers(new Set(onlineData.onlineUsers ||[]));
        } catch (onlineErr) {
          console.warn("⚠️ Online users endpoint not ready. Relying on Socket.io for presence.");
        }
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (socketRef.current) socketRef.current.emit('typing_stop', { receiverId: selectedChatUser._id, senderId: currentUserId });
    ...
    setDirectMessages(prev => [...prev, optimisticMsg]);
    setChatList(prev =>[{ ...selectedChatUser, lastMessage: messageToSend, lastMessageTime: new Date().toISOString() }, ...prev.filter(u => u._id !== selectedChatUser._id)]);
    ...
    try {
      if (editingMessageId) {
        ...
        const response = await api.post('/api/chat/send', { receiverId: selectedChatUser._id, message: messageToSend });
        setDirectMessages(prev => prev.map(msg => msg._id === tempId ? { ...response.data, isSending: false, isPending: false } : msg));
      }
    } catch (error) {
      toast.error("Failed to send message");
      setDirectMessages(prev => prev.filter(m => m._id !== tempId));
    ...
    <div className="flex w-full h-[calc(100vh-4rem)] bg-white font-sans mt-0">
      ...
      <div className="w-80 flex flex-col border-r border-gray-200 bg-white h-full">
        <div className="p-4 bg-gradient-to-r from-blue-600 to-indigo-700 text-white flex-shrink-0">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center"><FaCommentDots /></div>
              <div>
                <h2 className="font-bold text-lg">Employee Connect</h2>
                <p className="text-blue-100 text-xs opacity-80">{onlineUsers.size} online</p>
              </div>
            </div>
            {totalUnreadCount > 0 && <span className="bg-green-500 text-white text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full animate-pulse">{totalUnreadCount > 9 ? '9+' : totalUnreadCount}</span>}
          </div>
          <div className="relative group">
            <FaSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-blue-200" size={14}/>
            <input className="w-full pl-10 pr-4 py-2.5 bg-white/10 border border-white/20 rounded-xl text-sm outline-none placeholder:text-blue-100 text-white focus:bg-white/15 focus:border-white/30" placeholder="Search colleagues..." value={directSearchTerm} onChange={(e) => setDirectSearchTerm(e.target.value)} />
          </div>
        </div>
        ...
        <div ref={sidebarContainerRef} className="flex-1 overflow-y-auto" style={{ minHeight: 0 }}>
          <div className="px-3 py-2">
            {isListLoading ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-2 py-8"><FaSpinner className="animate-spin" size={20} /></div>
            ) : displayList.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400 p-4 text-center py-8"><FaUsers className="text-gray-300 mb-2" size={32} /><p className="text-sm font-semibold text-gray-500">No contacts found</p></div>
            ) : (
              displayList.map((emp) => {
                const isActive = selectedChatUser?._id === emp._id;
                const unreadCount = emp.unreadCount || 0;
                return (
                  <div key={emp._id} onClick={() => handleUserSwitch(emp)} className={`flex items-center gap-3 px-3 py-3 rounded-2xl cursor-pointer mb-1.5 transition-all duration-200 ${isActive ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white" : "hover:bg-gray-50"} ${isSwitchingUser ? "pointer-events-none opacity-70" : ""}`}>
                    <div className="relative">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-base flex-shrink-0 ${isActive ? "bg-white/20 border-2 border-white/30" : "bg-gradient-to-tr from-blue-500 to-indigo-500 text-white border-2 border-white"}`}>{emp.name?.charAt(0)}</div>
                      {onlineUsers.has(emp._id) && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-white"></div>}
                    </div>
                    <div className="flex-1 overflow-hidden min-w-0">
                      <div className="flex justify-between items-center mb-0.5"><p className={`text-sm font-bold truncate ${isActive ? "text-white" : "text-gray-800"}`}>{emp.name}</p></div>
                      <p className={`text-xs truncate font-medium ${isActive ? "text-blue-100" : "text-gray-500"}`}>{emp.lastMessage || emp.role || "No messages yet"}</p>
                    </div>
                    {unreadCount > 0 && !isActive && <span className="bg-green-500 text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full">{unreadCount > 9 ? '9+' : unreadCount}</span>}
                  </div>
                );
              })
            )}
        ...
        {selectedChatUser ? (
          <>
            <header className="h-16 flex items-center justify-between px-6 border-b border-gray-200 bg-white shadow-sm z-40 flex-shrink-0">
              <div className="flex items-center gap-4">
                <div className="relative">
                  <div className="w-12 h-12 bg-gradient-to-r from-blue-500 to-indigo-600 text-white flex items-center justify-center rounded-full font-bold text-lg">{selectedChatUser.name?.charAt(0)}</div>
                  {onlineUsers.has(selectedChatUser._id) && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 rounded-full border-2 border-white"></div>}
                </div>
                <div>
                  <h3 className="text-base font-bold text-gray-900">{selectedChatUser.name}</h3>
                  <div className="flex items-center gap-2">{typingUsers.has(selectedChatUser._id) ? <span className="text-xs text-blue-600 font-medium">typing...</span> : <span className="text-xs text-gray-500">{onlineUsers.has(selectedChatUser._id) ? "🟢 Online" : "⚫ Offline"}</span>}</div>
                </div>
              </div>
            </header>
            ...
            <div ref={chatContainerRef} className="flex-1 overflow-y-auto bg-gray-50" style={{ minHeight: 0 }}>
              {isSwitchingUser || isChatLoading ? (
                <div className="flex flex-col items-center justify-center h-full bg-white/80"><FaSpinner className="text-blue-600 animate-spin mb-3" size={30} /></div>
              ) : directMessages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-400"><div className="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center mb-4"><FaComments className="text-gray-300" size={32} /></div><h3 className="text-lg font-bold text-gray-600">Start a conversation</h3></div>
              ) : (
                <div className="p-6">
                  {Object.entries(groupedMessages).map(([date, msgs]) => (
                    <div key={date} className="mb-6">
                      <div className="flex justify-center my-6"><span className="bg-gray-200 text-gray-600 text-xs font-bold px-4 py-1.5 rounded-full uppercase tracking-wider">{date}</span></div>
                      {msgs.map((msg, index) => {
                        const isMe = msg.sender?._id === currentUserId || msg.sender === currentUserId;
                        const isMenuOpen = openMenuId === msg._id;
                        return (
                          <div key={msg._id || index} className={`flex w-full mb-4 ${isMe ? "justify-end" : "justify-start"}`}>
                            <div className={`max-w-[70%] group relative ${isMe ? "ml-auto" : ""}`}>
                              <div className={`flex flex-col ${isMe ? "items-end" : "items-start"}`}>
                                {!isMe && <span className="text-xs font-semibold text-gray-600 mb-1 px-2">{msg.sender?.name || selectedChatUser.name}</span>}
                                <div className="relative">
                                  <div className={`px-4 py-3 rounded-2xl shadow-sm ${isMe ? "bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-br-md" : "bg-white text-gray-800 border border-gray-200 rounded-bl-md"} ${msg.isPending ? "opacity-80" : ""}`}>
                                    ...
                                    <div className={`flex items-center justify-end gap-2 mt-2 ${isMe ? "text-blue-100" : "text-gray-500"}`}>
                                      ...
                                      {isMe && !msg.isPending && <FaCheckDouble className={`text-xs ${msg.isRead ? "text-green-300" : "opacity-60"}`} />}
                                    </div>
                                  </div>
                                  {isMe && !msg.isPending && (
                                    <>
                                      <button onClick={(e) => { e.stopPropagation(); setOpenMenuId(isMenuOpen ? null : msg._id); }} className="absolute -top-2 -left-2 w-6 h-6 bg-white border border-gray-200 rounded-full flex items-center justify-center text-gray-500 opacity-0 group-hover:opacity-100 hover:text-blue-600 z-10"><FaEllipsisV size={10} /></button>
                                      {isMenuOpen && (
                                        <div className="absolute left-0 top-6 bg-white border border-gray-100 shadow-xl rounded-lg py-1.5 z-50 w-32">
                                          <button onClick={() => { setEditingMessageId(msg._id); setDirectMsgText(msg.message); setOpenMenuId(null); }} className="w-full flex items-center gap-3 px-4 py-2 text-xs font-semibold text-gray-700 hover:bg-blue-50"><FaPen size={10} /> Edit</button>
                                          <button onClick={() => handleDeleteMessage(msg._id)} className="w-full flex items-center gap-3 px-4 py-2 text-xs font-semibold text-red-600 hover:bg-red-50"><FaTrash size={10} /> Delete</button>
                                        </div>
                                      )}
                                    </>
                                  )}
            ...
            <footer className="p-4 bg-white border-t border-gray-200 shadow-lg flex-shrink-0">
              {editingMessageId && (
                <div className="max-w-4xl mx-auto mb-3 flex items-center justify-between bg-blue-50 px-4 py-2.5 rounded-xl border border-blue-100"><div className="flex items-center gap-3"><FaRegEdit className="text-blue-600" /><span className="text-xs font-bold text-blue-800">Editing message...</span></div><button onClick={() => { setEditingMessageId(null); setDirectMsgText(""); }} className="text-blue-400 hover:text-blue-600"><FaTimes size={12} /></button></div>
              )}
              <div className="max-w-4xl mx-auto flex items-end gap-3 bg-gray-100 border border-transparent rounded-2xl p-3 focus-within:bg-white focus-within:border-blue-400">
                <div className="relative">
                  {showEmojiPicker && <div className="absolute bottom-16 left-0 z-50 shadow-2xl rounded-2xl overflow-hidden border border-gray-200"><EmojiPicker onEmojiClick={(e) => setDirectMsgText(prev => prev + e.emoji)} width={300} height={400} /></div>}
                  <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className={`p-3 rounded-xl ${showEmojiPicker ? "bg-blue-100 text-blue-600" : "text-gray-500 hover:text-blue-500"}`}><FaSmile size={20} /></button>
                </div>
                <textarea className="flex-1 bg-transparent outline-none resize-none text-sm py-2.5 px-3 text-gray-800 font-medium placeholder:text-gray-400 min-h-[44px] max-h-32" rows={1} placeholder="Type your message here..." value={directMsgText} onChange={(e) => handleTyping(e.target.value)} onKeyDown={handleKeyDown} disabled={sendingMessage || isSwitchingUser || isChatLoading} />
                <button onClick={handleSendMessage} disabled={!directMsgText.trim() || sendingMessage} className={`p-3.5 rounded-xl shadow-md flex items-center justify-center ${directMsgText.trim() && !sendingMessage ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white" : "bg-gray-200 text-gray-400"}`}>
          ...
          <div className="flex flex-1 flex-col items-center justify-center text-gray-400"><FaUsers size={48} className="opacity-60 mb-6"/><h3 className="text-xl font-bold text-gray-800">Connect with your team</h3><p className="text-sm">Select a colleague from the sidebar to start a chat.</p></div>
        )}
      </div>
    </div>
  );
