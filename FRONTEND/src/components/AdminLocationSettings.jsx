...
const ScheduleModal = ({ isOpen, onClose, employee, onSave }) => {
  ...
  const handleSave = () => {
    ...
    if (activeTab === "Temporary") {
      if (!fromDate || !toDate)
        return Swal.fire("Error", "Please select From and To dates.", "error");
      ...
      if (recurDays.length === 0)
        return Swal.fire("Error", "Select at least one day.", "error");
      ...
      <div className="bg-white rounded-2xl p-6 w-full max-w-lg shadow-2xl border border-gray-100 flex flex-col">
          ...
          {activeTab === "Temporary" && (
            <div className="space-y-4 animate-fade-in">
              <div className="bg-blue-50 p-3 rounded-lg text-xs text-blue-700 mb-2">
                Set a temporary mode. Automatically reverts to Global settings
                after the "To Date".
              </div>
              <div>
          ...
          {activeTab === "Recurring" && (
            <div className="space-y-4 animate-fade-in">
              <div className="bg-purple-50 p-3 rounded-lg text-xs text-purple-700 mb-2">
                Set specific days to have a fixed mode. Other days follow Global
                settings.
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1">
          ...
          {activeTab === "Permanent" && (
            <div className="space-y-4 animate-fade-in">
              <div className="bg-orange-50 p-3 rounded-lg text-xs text-orange-700 mb-2">
                Permanently override Global settings for this employee until
                changed manually.
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">
                ...
                <div className="flex flex-col gap-2">
                  {["WFO", "WFH"].map((m) => (
                    <label
                      key={m}
                      className={`flex items-center gap-3 p-3 border rounded-xl cursor-pointer transition-all ${permMode === m ? "bg-blue-50 border-blue-500 ring-1 ring-blue-500" : "hover:bg-gray-50 border-gray-200"}`}
                    >
                      <input
                        type="radio"
                        name="permMode"
                      ...
                      <span className="font-semibold text-gray-700">
                        {m === "WFO" ? "Work From Office" : "Work From Home"}
                      </span>
                    </label>
                  ))}
                </div>
        ...
        <div className="flex justify-between pt-4 border-t">
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleSave}
              className="px-6 py-2 bg-gray-900 text-white text-sm font-bold rounded-lg hover:bg-black transition"
            >
              Save Rule
            </button>
          </div>
...
const AdminLocationSettings = () => {
      ...
      if (data) {
        setSettings({
          ...
          requireAccurateLocation:
            data.requireAccurateLocation !== undefined
              ? data.requireAccurateLocation
              : true,
        });
        if (data.officeLocation?.latitude) {
          setMapCenter([
            data.officeLocation.latitude,
            data.officeLocation.longitude,
          ]);
          setSelectedCoords([
            data.officeLocation.latitude,
            data.officeLocation.longitude,
  ...
  const confirmLocation = () => {
    if (selectedCoords) {
      ...
      Swal.fire(
        "Select Location",
        "Please click on the map to place a pin.",
        "warning",
      );
    }
  };
  ...
  const handleSaveGlobalSettings = async () => {
    if (
      ...
      return Swal.fire(
        "Warning",
        "Please set Latitude and Longitude for WFO with accurate location enabled.",
        "warning",
      );
    }
  ...
  const handleBulkUpdate = async (mode) => {
    try {
      ...
      await api.post("/api/admin/settings/employee-mode/bulk", {
  ...
  const handleResetAll = () => {
    Swal.fire({
      ...
      text: "Everyone will revert to Global Settings.",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#d33",
  ...
  const handleSaveCategory = async (name, ids) => {
    try {
      ...
      await api.post("/api/admin/settings/categories", {
        name,
        employeeIds: ids,
      });
  ...
  const handleAddMembersToCategory = (newIds) => {
    const currentCategoryEmployees = employees.filter(
  ...
  const handleRemoveFromCategory = async (employee) => {
    try {
      ...
      Swal.fire(
        "Removed",
        `${employee.name} moved to Uncategorized.`,
        "success",
      );
      fetchEmployees();
    } catch (err) {
  ...
  return (
    <div className="p-6 min-h-screen font-sans relative">
      <div className="max-w-7xl mx-auto space-y-6">
        {/* Global Settings */}
        <div className="overflow-hidden bg-white/20 backdrop-blur-md shadow-sm rounded-2xl border border-gray-300">
          <div className="px-6 m-6 mb-0 py-4 bg-white border border-gray-300 rounded-2xl justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Base office settings and default mode.
                </p>
              </div>
            </div>
          </div>
          <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            <div className="space-y-6 border border-gray-300 bg-white p-4 rounded-2xl">
              <div>
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl">
                  <label
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg cursor-pointer transition-all ${settings.globalWorkMode === "WFO" ? "bg-white text-blue-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                  >
                    <input
                      type="radio"
                      value="WFO"
                  ...
                  <label
                    className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg cursor-pointer transition-all ${settings.globalWorkMode === "WFH" ? "bg-white text-green-700 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}
                  >
                    <input
                      type="radio"
                      value="WFH"
                    ...
                    <div className="relative inline-block w-12 mr-2 align-middle select-none">
                      <input
                        ...
                        onChange={(e) =>
                          setSettings({
                            ...settings,
                            requireAccurateLocation: e.target.checked,
                          })
                        }
                        className="sr-only"
                      />
                      ...
                      >
                        <div
                          className={`h-6 w-6 rounded-full bg-white shadow transform transition-transform duration-200 ${settings.requireAccurateLocation ? "translate-x-6" : "translate-x-0"}`}
                        ></div>
                      </label>
                    </div>
                  </div>
                ...
                {settings.globalWorkMode === "WFO" &&
                  !settings.requireAccurateLocation && (
                    <p className="text-xs text-gray-500 mt-1">
                      Radius setting is disabled when location enforcement is
                      off.
                    </p>
                  )}
              </div>
            </div>
            <div className="space-y-6 bg-white p-4 rounded-2xl border border-gray-300">
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="relative">
                    <span className="absolute inset-y-0 left-3 flex items-center pointer-events-none text-gray-400 text-[10px] font-bold">
                      LAT
                    </span>
                    <input
                      type="number"
                      ...
                      className={`w-full pl-11 p-2.5 ${!settings.requireAccurateLocation ? "bg-gray-100" : "bg-white"} border border-gray-200 rounded-lg font-mono text-sm focus:ring-2 focus:ring-blue-500 outline-none`}
                      placeholder="0.0000"
                      disabled={!settings.requireAccurateLocation}
                    />
                  </div>
                ...
                {!settings.requireAccurateLocation && (
                  <p className="text-xs text-gray-500 mt-2 italic">
                    Coordinates are not required when location enforcement is
                    disabled.
                  </p>
                )}
              </div>
              <button
                ...
                disabled={loading}
                className="w-full bg-gray-900 text-white py-2.5 rounded-lg font-bold text-sm hover:bg-black transition shadow-sm flex justify-center items-center gap-2"
              >
                {loading ? (
                  "Saving..."
                ) : (
        ...
        {/* Employee Settings */}
        <div className="bg-white/20 backdrop-blur-md shadow-sm rounded-2xl border border-gray-300 flex flex-col min-h-[600px] overflow-hidden">
          <div className="p-6 pb-2">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 bg-white border border-gray-300 p-4 rounded-2xl shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Manage individual exceptions and categories.
                  </p>
                </div>
              </div>
      ...
      <AddMemberModal
        ...
        onAdd={handleAddMembersToCategory}
        allEmployees={employees}
