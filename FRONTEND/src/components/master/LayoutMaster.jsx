...
import {
  ...
  Sliders,
  LifeBuoy,
  ChevronRight, Bug, CalendarCheck, User
} from "lucide-react";
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.path}
                to={item.path}
                className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 group whitespace-nowrap ${isActive
