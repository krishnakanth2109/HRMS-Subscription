...
const initBirthdayCron = async () => {
  let cron = null;
  try {
    const cronModule = await import("node-cron");
    cron = cronModule.default || cronModule;
  } catch (err) {
    // node-cron is not installed in node_modules, seamlessly fall back to native timer
  }

  if (cron && typeof cron.schedule === "function") {
    cron.schedule("0 0 * * *", runBirthdayJob);
    console.log("🕒 Birthday Cron Job Initialized via node-cron (runs daily at 12:00 AM)");
  } else {
    // Zero-dependency native timer fallback: schedule run at midnight daily
    const now = new Date();
    ...
    console.log(`🕒 Birthday Timer Initialized (next run scheduled at midnight)`);
  }
};
